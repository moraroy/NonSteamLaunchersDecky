(function (deckyFrontendLib, React) {
  ...
  */
  ...
  function initGameWatcher() {
      ...
      const steamAppStore = appStore;
      const runGameLEDPalette = async (gameId, gameName) => {
          const generation = (steamWindow.__watcherLedGeneration ?? 0) + 1;
          steamWindow.__watcherLedGeneration = generation;
          try {
              const [PALETTE_SIZE, QUANTIZE, MIN_DISTANCE, MAX_SAMPLES] = [5, 16, 55, 2e4];
              const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));
              const req = steamWindow.__steam_require ||
                  await new Promise((resolve, reject) => {
                      if (typeof steamWindow.webpackChunksteamui !==
                          "object") {
                          reject(Error("Steam Webpack runtime not available"));
                          return;
                      }
                      let done = 0;
              ...
              const color = (id, i, c) => LED.SetColor({
                  device_id: id,
                  color_index: i,
                  color: c
              });
              const effect = (id, e) => LED.SetEffect({
                  device_id: id,
                  effect: e
              });
              const apps = steamAppStore.allApps.filter(a => a?.app_type === 1073741824);
              const gid = String(gameId);
              const app = apps.find(a => String(a?.m_gameid) === gid) ||
                  apps.find(a => String(a?.appid) === gid);
              if (!app) {
                  throw Error(`Non-Steam game not found: ${gameName || gid}`);
              }
              const { appid, display_name, icon_data, icon_data_format } = app;
              if (!icon_data) {
                  throw Error(`No icon_data found for ${display_name ||
                    gameName ||
                    gid} (${appid})`);
              }
              ...
              if (!ctx) {
                  throw Error("Failed to acquire canvas 2D context");
              }
              ...
              const q = (v) => Math.floor(v / QUANTIZE) *
                  QUANTIZE +
                  QUANTIZE / 2;
              ...
              if (!colors.size) {
                  throw Error("Icon contains no usable pixels");
              }
              ...
              if (!palette.length) {
                  throw Error("Could not extract a usable color palette");
              }
              ...
              if (!steamWindow.__watcherLedSnapshot) {
                  steamWindow.__watcherLedSnapshot =
                      devices.map(d => ({
                          id: d.id,
                          effect: d.effect,
                          colors: (d.color ?? []).map(({ r, g, b }) => ({
                              r,
                              g,
                              b
                          }))
              ...
              await Promise.all(devices.flatMap(d => {
                  const n = d.color?.length ??
                      0;
                  return Array.from({
                      length: n
                  }, (_, i) => color(d.id, i, ledPalette[Math.min(Math.floor(i *
                      palette.length /
                      n), ledPalette.length -
                      1)]));
              }));
              ...
              const snapshot = steamWindow.__watcherLedSnapshot;
              if (!snapshot) {
                  return;
              }
              await Promise.all(snapshot.flatMap(s => {
                  ...
                  try {
                      ...
                      if (line.includes("OnGameActionUserRequest") &&
                          ...
                          if (m &&
                              ...
                              m[1].length <=
                                  20) {
                              const app = steamAppStore.allApps.find(a => String(a.m_gameid) ===
                                  m[1]);
                              if (app?.app_type ===
                                  1073741824) {
                                  desktopGameRunning =
      ...
      (() => {
          const MIN_STATE_GAP_MS = 1e3, RAPID_TRANSITION_MS = 250, TERMINATION_DELAY_MS = 1e4, EXIT_CONFIRMATION_MS = 1e4, MIN_GAME_RUNTIME_MS = 3e4;
          const s = {
              gameId: null,
              gameName: null,
              launchTime: 0,
          ...
          const clearExit = () => {
              if (s.exitConfirmationTimer) {
                  clearTimeout(s.exitConfirmationTimer);
              }
          ...
          try {
              ...
              if (!currentConsoleLog.__steamDetectQAMHook) {
          ...
          try {
              const original = SteamClient.Overlay
                  .SetOverlayState;
              SteamClient.Overlay.SetOverlayState =
                  function (gameId, stateNum) {
                      if (!s.watchersEnabled ||
                          String(gameId) !==
                              String(s.gameId)) {
