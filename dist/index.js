(function (deckyFrontendLib, React) {
      ...
      return ((progress.status !== '' && progress.percent < 100) ? (window.SP_REACT.createElement(deckyFrontendLib.ModalRoot, null,
          ...
          window.SP_REACT.createElement(deckyFrontendLib.DialogBody, null,
              window.SP_REACT.createElement("div", { style: { display: 'flex', flexDirection: 'row', gap: '1em', marginBottom: '1em', alignItems: 'center' } },
                  ...
                  window.SP_REACT.createElement(deckyFrontendLib.ToggleField, { label: "Mozilla Firefox", checked: selectedBrowser === "Mozilla Firefox", onChange: () => setSelectedBrowser("Mozilla Firefox") }),
  ...
  */
  ...
  const isValidEntry = (e) => e && typeof e.total === "number" && typeof e.lastSessionEnd === "number";
  const loadPlaytimeData = () => {
      if (playtimeCache)
  ...
  const applyRealSessionToOverview = (ov) => {
      if (!ov || ov.app_type !== 1073741824)
          return false;
  ...
  const patchAppStore = () => {
      if (!window.appStore?.m_mapApps || appStore.m_mapApps._originalSet)
          return;
  ...
  const patchAppInfoStore = () => {
      if (!window.appInfoStore || appInfoStore._originalOnAppOverviewChange)
          return;
  ...
  const initRealPlaytime = (retry = 0) => {
      if (!isEnvironmentReady()) {
          if (retry < 100)
              setTimeout(() => initRealPlaytime(retry + 1), 1000);
          return;
      }
      try {
          restoreSavedPlaytimes();
